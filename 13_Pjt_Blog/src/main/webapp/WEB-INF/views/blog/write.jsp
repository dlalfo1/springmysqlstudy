...
<script>
	...
	// javascript 부분을 따로 파일로 빼서 포함시키는 걸로 프젝을 진행할 경우 ${contextPath} 이 부분을 인식하지 못한다.
	...
	$(function(){	// 로드이벤트
		$('#content').summernote({
			...
			callbacks: {					   				// summernote 올렸을 때 onImageUpload 이벤트가 작동한다.
			  onImageUpload: function(files) { 				// summernote 편집기에 이미지를 올리면 해당 이미지 정보는 function의 매개변수 files로 전달된다.
				  for(let i = 0; i < files.length; i++) {	// fils 배열을 순회하면서 이미지를 하나씩 처리한다.
					  var formData = new FormData();	    // 가상 form (주로 ajax처리에서 사용)
					  formData.append('file', files[i]);	// 가상 form에 추가한 요소
					$.ajax({								// 가상 form에 저장된 이미지를 HDD에 저장하고 저장된 경로와 (매핑)와 이름을 받아오는 ajax
						...
						success: function(resData){		// resData = {"src": "/app13/imageLoad/abcdefg.jpg"}
							$('#content').summernote('insertImage', resData.src);	// content에 <img src="/app13/imageLoad/abcdefg.jpg"> 태그가 추가 된다.
																					// content에 추가된 img 태그의 src 속성 값은 servlet-context.xml의 resources 태거의 의해서 /storage/summernote/abcdefg.jpg로 처리된다.
						}
